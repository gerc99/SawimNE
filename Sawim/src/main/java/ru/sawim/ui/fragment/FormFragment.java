 ...
 */
public class FormFragment extends DialogFragment implements Forms.OnUpdateForm, DialogInterface.OnCancelListener, DialogInterface.OnDismissListener {
    ...
    public FormFragment init(Forms forms) {
        this.forms = forms;
        return this;
    }
            ...
            activity.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (!activity.isFinishing()) {
                        new FormFragment().init(f).show(activity.getSupportFragmentManager().beginTransaction(), "form");
                    }
                }
            });
        }
    ...
    public static void showWindows(final BaseActivity activity, final String title) {
        activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (!activity.isFinishing()) {
                    FormFragment formFragment = new FormFragment().init(formsMap.get(title));
                    formFragment.show(activity.getSupportFragmentManager().beginTransaction(), "form");
                }
            }
        });
    }
