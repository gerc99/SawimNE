...
import protocol.mrim.MrimContact;
            ...
            case ACTION_MESSAGE_TEXT:
                ...
                if (track_prev!=null) {
                    if (track_prev.idAction==ACTION_MESSAGE) {
                        ...
                        if (str.length()!=0) ContactList.getInstance().getManager().getCurrentProtocol().sendMessage(item, str, true);
                    }
                }
                break;
        }
    ...
    private static void beginTrackActionStatus(Contact item, int event, int action) {
        ...
        switch (action) {
            case ACTION_INCHAT:
                ...
                plainMsg.setSendingState(Message.ICON_MSG_TRACK);
                plainMsg.setStatusIcon(icon_status);
                plainMsg.setStatusName(status_name);
